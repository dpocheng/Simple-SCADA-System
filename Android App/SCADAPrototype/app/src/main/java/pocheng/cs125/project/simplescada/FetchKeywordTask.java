...
public class FetchKeywordTask extends AsyncTask<String, Void, ArrayList<DeviceInfo>> {
    private final String LOG_TAG = FetchKeywordTask.class.getSimpleName();
    ...
    protected ArrayList<DeviceInfo> doInBackground(String... params) {
        ...
        try {
            final String BASE_URL = "http://35.167.243.164/get?";
            final String AREA_PARAM = "area";
            final String DEVICE_PARAM = "device";

            Uri builtUri;
            if (params[1].equals("empty")) {
                builtUri = Uri.parse(BASE_URL).buildUpon()
                        .appendQueryParameter(AREA_PARAM, params[0])
                        .build();
            }
            else {
                builtUri = Uri.parse(BASE_URL).buildUpon()
                        .appendQueryParameter(AREA_PARAM, params[0])
                        .appendQueryParameter(DEVICE_PARAM, params[1])
                        .build();
            }
